  ...
  return (
    <div>
      {tachesAvecDates.map((tache) => (
        <div className="card">
          <div key={tache.id} className={`chk-item ${tache.enRetard ? 'chk-item--late' : ''} ${tache.enUrgence ? 'chk-item--urgent' : ''}`}>
          <span>{tache.titre}</span>
          <span>{tache.description}</span>
          <span>{tache.type}</span>
          <span>{tache.dateEcheance.toLocaleDateString('fr-FR')}</span>
          {tache.enRetard && <span>En retard</span>}
        </div>
        </div>
      ))}
    </div>
